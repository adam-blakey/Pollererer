<?php
	...
	$poll_hidden     = htmlspecialchars($_POST["poll_hidden"]);
	...
	{      
		...
		if ($user_level >= 1)
		{
			// Firstly, set the visibility of the poll.
			$update_hidden_query = $db_connection->query("UPDATE `terms` SET `hidden`='".$poll_hidden."' WHERE `ID`='".$term_ID."'");

			if (!$update_hidden_query)
			{
				$JSON_response->status        = "error";
				$JSON_response->error_message = "failed to update hidden status of poll; ".$db_connection->error;
